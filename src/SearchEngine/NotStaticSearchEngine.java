...
public class NotStaticSearchEngine implements Serializable {
	...
	public HashSet<String> createTrie(String urlName) throws IOException {
		// URL passed from Console.java to fetch all the all links to a certain depth
		...
		// here we are making call to crawler to find all the links
	...
	{
		// System.out.println("inside notstatic Search method");
		int[] count = new int[allLinks.size()]; // array of counts
		// size is count of all the links
		List<String> links = new ArrayList<String>(allLinks); // list of links

		ArrayList<Integer> tmp = null;
		for (int i = 0; i < index.length; ++i) {
			tmp = trie.searchWord(index[i].toLowerCase());
			// System.out.println("trie return object: "+tmp);

			if (tmp != null) {
				// if word is not found in tree then suggest words
